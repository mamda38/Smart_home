...
// API to get sensor log
app.get('/api/sensor_log', (req, res) => {
    db.query('SELECT * FROM sensor_readings ORDER BY timestamp DESC LIMIT 100', (err, results) => {
        if (err) {
...
// API to get LED command log
app.get('/api/action_log', (req, res) => {
    db.query('SELECT * FROM led_command_log ORDER BY timestamp DESC LIMIT 100', (err, results) => {
        if (err) {
